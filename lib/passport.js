...
passport.deserializeUser(async (id, done) => {
  try {
    ...
    // Treat DB errors as "user not found" rather than crashing the request.
    // This prevents 500s on all pages when the DB is temporarily unreachable.
    console.error('[passport] deserializeUser error (treating as logged-out):', err.message);
    done(null, null);
  }
});
