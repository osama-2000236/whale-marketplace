...
function getOrCreateCart(userId) {
  ...
  let cart = store.cartsByUserId.get(userId);
  if (!cart) {
    cart = {
      id: `cart-${userId}`,
      userId,
    ...
    store.cartsByUserId.set(userId, cart);
  }
  return cart;
}
...
function listCartItems(userId) {
  ...
  return cart.items
    .map((item) => ({
      ...item,
      listing: findListingBySlugOrId(item.listingId),
    }))
    .filter((item) => Boolean(item.listing));
}

function upsertCartItem(userId, listingId, quantity) {
  const cart = getOrCreateCart(userId);
...
function createOrder({
  ...
  order.events[0].orderId = order.id;
  getStore().ordersById.set(order.id, order);
  return order;
}

function findOrderById(orderId) {
  return getStore().ordersById.get(orderId) || null;
}

function listOrdersForUser(userId, tab = 'buying') {
  const orders = Array.from(getStore().ordersById.values()).filter((order) =>
    tab === 'selling' ? order.sellerId === userId : order.buyerId === userId,
  );
